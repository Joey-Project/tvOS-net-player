...
- The eighth PR 6B independent-review follow-up distinguishes transient persistence failures from a malformed startup snapshot whose writable store is detached for the process. Read traffic now repairs retained-store failures, terminal download and playback-planning owners wait only while in-process recovery is possible, and detached stores release volatile work permits without enabling v2 or destructive cache deletion.
- The ninth PR 6B independent-review follow-up installs a completed HLS runtime as soon as its media and manifest are durable, then reuses that completion while task-state persistence retries instead of downloading or transcoding again. Nested local-media deletion races remain idempotent when an already validated intermediate directory disappears, while root and access-policy failures stay explicit.
- The tenth PR 6B independent-review follow-up compacts legacy-managed v2 persistence so v1 result bodies are not duplicated during automatic writeback, while validating and reconstructing the same generic projection on load. The periodic HLS maintenance owner now remains active with unlimited cache policy and retries process-local physical cleanup intents before the disabled-eviction return.
- The eleventh PR 6B independent-review follow-up separates task-resource body ownership from HTTP authorization so startup and cleanup retain `PENDING`, `UNAVAILABLE`, and not-yet-durably-retired expired bodies. Manual completed-HLS deletion now shares the quota snapshot lock before taking the deletion lock, and successful task-resource responses require private-cache revalidation with `Cache-Control: private, no-cache`.
- The twelfth PR 6B independent-review follow-up rejects ASCII case aliases of the managed cache namespace, rechecks resource expiry after serialized open work, degrades v2 on authorized resource-storage failures until exact-ID revalidation succeeds, and moves task-result pagination/reaping plus terminal download persistence off Tokio workers. Startup retains non-serving HLS finalizer ownership across transient initial writeback failure, while HLS completion keeps its finalizer queued until parent-directory synchronization is durable.
- The thirteenth PR 6B self-review follow-up keeps an already-complete restored HLS session on the queued recovery path while initial task persistence is unavailable, so finalizer ownership survives without republishing runtime playback before publication durability recovers.
- The fourteenth PR 6B independent-review follow-up restores pending resource-cleanup reservations when a durability-required registry mutation rolls back, preventing rejected retries from accumulating phantom resource IDs. Startup also retains the complete HLS inventory across a transient task-state rewrite failure and removes still-unauthorized orphan sessions once task persistence recovers, including sessions that have no finalizer owner.
- The fifteenth PR 6B independent-review follow-up rejects cross-task output snapshot/resource identity collisions before startup can rewrite a survivor-only snapshot, waits for actual finalizer ownership to end before reauthorizing restored HLS sessions, and retries transient unauthorized-session deletion failures even when quota eviction is disabled or below its high watermark.
- The sixteenth PR 6B independent-review follow-up extends persisted identity collision rejection to restore-normalized task IDs and serializes finalizer eviction-protection registration with completed and partial HLS physical deletion, closing the last check-to-delete race during quota enforcement.
- The seventeenth PR 6B independent-review follow-up hands initial unauthorized-HLS deletion failures to startup reconciliation even when persistence is already healthy, preventing invisible disk orphans without registering them for serving. Task-result page snapshots now carry the resource lease's exact monotonic deadline, reject already-expired first-page publication, and cannot advertise a continuation token after the retained resource lease ends.
- The eighteenth PR 6B independent-review follow-up registers staged output resource IDs as reference-counted body owners before expiry retirement can recover persistence and rerun startup orphan cleanup, preventing a newly written body from being deleted before its metadata commit. Pending HLS finalizer restore and reconciliation work is now shared across `AppState` clones when construction occurs outside Tokio; every public server entry starts it idempotently after entering a runtime, and retry sessions remain owned across worker cancellation.
- The nineteenth PR 6B independent-review follow-up replaces metadata-only resource publication with a two-phase claim that serializes ownership registration against cleanup before callers can create bodies, then consumes the claim for durable metadata commit. Startup-reconciliation HLS session IDs are quarantined from HTTP serving, lazy runtime registration, completed-cache source authorization, and playback-progress authorization until durable recovery classifies or deletes them.
- The twentieth PR 6B review follow-up exposes only newly allocated resource IDs for producer body creation. Identical current-task representations remain immutable and reusable, while duplicate IDs and representation changes fail during staging before any live body can be overwritten.
- The twenty-first PR 6B independent-review follow-up makes immutable result-page leases reserve every referenced resource ID, including pending and unavailable artifacts, without broadening media authorization beyond available artifacts. Task-terminal HLS cleanup now removes runtime authorization immediately and records failed physical deletions for maintenance retry even when quota eviction is disabled or below its high watermark.
- The twenty-second PR 6B independent-review follow-up keeps first-page snapshots unpublished until an RPC receives its detached blocking result. Concurrent requests register independently; cancellation withdraws only that request, while the final unpublished withdrawal removes the snapshot and releases its current resource lease immediately.
- The twenty-third PR 6B independent-review follow-up requires every HLS completion recovery attempt to reach parent-directory durability, moves the initial failed/cancelled playback-planning transition onto Tokio's blocking pool, and prevents capacity pressure from evicting a first-page snapshot before RPC publication. A request that cannot fit while unpublished snapshots are protected now releases its own lease and returns bounded backpressure.
...
- The twenty-eighth PR 6B independent-review follow-up removes queued HLS fills and waits for an active fill to release scheduler ownership before task cancellation physically deletes the same session directories.
- The twenty-ninth PR 6B independent-review follow-up preflights a conservative 128 MiB aggregate task-record clone budget under the registry lock, serializes that bounded snapshot one borrowed record at a time after releasing the registry lock, classifies deterministic validation/schema/size failures as permanent, and proves that a single-concurrency download worker starts its next queued task when an identical retry cannot recover while preserving retries for filesystem and directory-sync failures.
- The thirtieth PR 6B independent-review follow-up restores the exact task, generic output, and pending publication when a playback-result update is permanently rejected. Persistence recovery now carries its durable/retryable/permanent outcome through playback planning, HLS fill, and startup reconciliation so deterministic rejection releases every background permit or ownership slot while transient filesystem failures retain their existing retry behavior.
- The thirty-first PR 6B independent-review follow-up makes pre-finalization quota enforcement fail closed. A failed cache scan, task tombstone, or physical eviction now stops the full resource fill, demotes and requeues the scheduler job, and resumes only after the quota path recovers.
- The thirty-second PR 6B independent-review follow-up fences task cancellation against single-result publication, explicit multi-result publication, and startup finalizer restoration. A shared synchronous lifecycle lock covers committed task/session revalidation, runtime/manifest publication, final `PLAYABLE` or restored `COMPLETED` transition, and every scheduler enqueue; cancellation commits its task transition and signals or removes fill work under the same lock before waiting and deleting session directories. A cancellation that wins first rejects late child or restored-session publication without recreating bytes, while a producer that wins first must finish enqueueing before cancellation can quiesce and clean it.
- The thirty-third PR 6B independent-review follow-up restores task, generic output, pending publication, and playback-planning ownership when a final planned/playable publication is permanently rejected, so a later valid persistence retry can restore v2 without restart. Manual completed-HLS deletion now cancels the exact primary or secondary session set under the shared lifecycle fence, waits for any matching active fill to exit without holding synchronous locks, then revalidates authorization before committing the task tombstone and removing cache bytes; deleting one completed secondary result preserves unrelated sibling fills.
- The thirty-fourth PR 6B CI follow-up gives every gRPC fixture that overrides the cache root a matching fixture-local task-state path. Parallel tests can no longer race through the shared executable-relative `tasks.json.tmp` publication path and reject otherwise valid playback-task creation with a transient persistence failure.
...
- PR 6B server coverage: `CacheServer/RustCacheServer/src/task_output.rs`, `task_store.rs`, `task_registry.rs`, `hls_fill_scheduler.rs`, `grpc_services.rs`, `library.rs`, and `media.rs`; the post-review Rust suite passes 690 unit tests, 34 live-e2e helper tests, and 6 integration tests, with 1 opt-in real-network case ignored.
