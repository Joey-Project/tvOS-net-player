...
- Final Rust gate: 480 unit tests, 34 deterministic live-harness tests plus 1 ignored opt-in live test, and 6 integration tests passed.
- Final official live run: ordinary video, multi-part video, space videos/collection, homepage recommendations, authenticated history, and authenticated watch-later passed in 325.69 seconds.
- Final restricted live run: Bangumi media series and episode passed in 12.52 seconds with a temporary access-key-only profile; the temporary credential root was removed after the run.
...
- `cargo test -p tvos-net-player-cache-server credential_configured_rpc_ --locked`
- `cargo test -p tvos-net-player-cache-server task_client_boundary_redacts_cache_fill_failure_from_playable_parent_states --locked`
