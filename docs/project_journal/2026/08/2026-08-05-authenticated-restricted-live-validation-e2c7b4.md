...
- Kept completed HLS playlists and persisted manifests upstream-free while allowing already-issued alternate playlist and range URLs to finish from in-memory runtime metadata during one fixed 60-second absolute grace period. The registry stores the deadline and enforces it on lookup before serving media; a detached timer only performs eager cleanup. The generation-bound scrub prevents either path from overwriting even a byte-for-byte identical newer same-id session.
