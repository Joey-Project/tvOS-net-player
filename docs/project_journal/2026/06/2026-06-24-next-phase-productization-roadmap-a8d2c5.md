...
- Added named credential-profile selection, per-case server isolation, aggregate failures, credential-safe harness/background-log output, and teardown that first stops and awaits both listeners, then enumerates and cancels every task in the isolated registry before waiting for terminal tasks plus stable registered background-work quiescence. This covers lost create responses as well as tracked tasks. Scheduler completion/requeue is atomic with cancellation, and teardown closes and awaits the per-state fill worker, so terminal multi-result parents cannot leave secondary fills or credential-bearing clients alive against a dropped live-test root.
