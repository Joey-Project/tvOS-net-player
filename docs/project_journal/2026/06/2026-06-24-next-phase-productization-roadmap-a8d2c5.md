...
- Added named credential-profile selection, per-case server isolation, aggregate failures, credential-safe harness/background-log/RPC output whenever a credential path is configured, and teardown that first stops and awaits both listeners, then enumerates and cancels every task in the isolated registry before waiting for terminal tasks plus stable registered background-work quiescence. This covers credential-load failures, legacy persisted failures, lost create responses, and tracked tasks without leaking upstream details. Scheduler completion/requeue is atomic with cancellation, and teardown closes and awaits the per-state fill worker, so terminal multi-result parents cannot leave secondary fills or credential-bearing clients alive against a dropped live-test root.
...
- Bound stable collection-item ids to the canonical parsed collection source so an id from another list/feed endpoint is rejected instead of recovered as unrelated content.
- Fixed completed-HLS alternate-playlist and range races without persisting upstream URLs or headers: hidden runtime alternate requests receive a fixed 60-second absolute compatibility grace whose deadline is enforced on registry lookup, then an atomic generation-bound scrub removes them without touching a newer same-id session. A detached timer remains an eager cleanup optimization rather than the enforcement boundary.
