...
- Segment durations come from fMP4 timing metadata; multi-track fragments sum repeated `traf` durations per track and use the longest parsed track duration, while split playlist output is disabled when timing metadata is unavailable.
...
- Review hardening also bounds timing metadata payload reads; oversized `moov`/`moof` boxes fall back to no segment index.
