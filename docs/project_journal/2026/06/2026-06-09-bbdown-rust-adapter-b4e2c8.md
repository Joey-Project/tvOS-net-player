...
- Materialized BBDown mux outputs as local cache library item ids so completed tasks can be played through the existing HTTP Range endpoint.
...
- The adapter enables `MuxOptions::ffmpeg(...)` because the current library indexes complete `.mp4`, `.m4v`, and `.mov` assets, not raw DASH media sidecars.
...
- Add a live local smoke test path once a stable Bilibili test asset and local `ffmpeg` dependency policy are agreed.
