...
- The policy can instead preserve BBDown's requested/default variant ranking, prevent or force LAN transcoding, hold a weak-network downgrade for the rest of the session, or leave ABR decisions entirely to AVPlayer. `BilibiliPlaybackSession.effective_policy` reports the normalized policy actually attached to the HLS session; task snapshots and HLS manifests persist it with backward-compatible defaults.
