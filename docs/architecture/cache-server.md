...
- `TaskService` accepts Bilibili URL/BV task intake into a persisted queue, returns active duplicate submissions as the same task, streams task snapshots and updates through `WatchTasks`, and supports idempotent queued cancellation plus running/preparing cancellation requests. Progressive playback task creation returns a persisted `preparing` task immediately; BBDown playback planning runs in the background, then publishes `planned` `BilibiliPlaybackSession` metadata through `GetTask`/`WatchTasks`. It does not expose a playable HLS `PlaybackSource` until the media pipeline can serve the route.
