...
- Progressive playback accepts a per-request `BilibiliPlaybackPolicy` behind `SERVER_CAPABILITY_BILIBILI_PLAYBACK_POLICY`. The safe default uses automatic transcoding, prefers a variant already within the conservative AVPlayer H.264/AAC HLS profile, and lets the server adaptively downgrade and recover weak upstream variants. Explicit H.264/HEVC/AV1 preferences still take precedence over compatible-first ranking.
