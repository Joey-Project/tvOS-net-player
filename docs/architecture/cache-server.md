...
- LAN transcoding foundation is exposed as a control-plane and persistence boundary only. `Cache:LanTranscodingEnabled` defaults to `false`; when enabled, `ServerService.GetServerInfo` includes `SERVER_CAPABILITY_LAN_TRANSCODING`, and `CacheService.GetHlsCacheStatus` reports a `LanTranscodingStatus` with the conservative `avplayer-h264-aac-hls-v1` H.264/AAC HLS/fMP4 target profile. Each progressive `BilibiliPlaybackSession` and HLS session manifest can persist a `LanTranscodingPlan` with `disabled`, `not_required`, `ready`, or `unsupported` state. The foundation does not start ffmpeg, schedule jobs, or rewrite playlists yet.
...
- `Cache:LanTranscodingEnabled`: enables the LAN transcoding control-plane foundation and advertises `SERVER_CAPABILITY_LAN_TRANSCODING`. Defaults to `false`.
- `Cache:LanTranscodingFfmpegPath`: `ffmpeg` executable path reserved for future LAN transcoding execution. Defaults to `ffmpeg` from `PATH`.
- `Cache:LanTranscodingMaxConcurrentJobs`: maximum future LAN transcoding job concurrency. Defaults to `1`; must be greater than zero.
