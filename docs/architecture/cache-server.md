...
- When `Cache:BonjourEnabled=true` and the gRPC listener includes a non-loopback address, the Rust server publishes `_tvos-net-player._tcp.local.` through mDNS with TXT metadata for `server_id`, `server_name`, and server `version`. Bonjour publication is best-effort: if mDNS registration fails, gRPC and media listeners still start and the apps can use manual host entry. The default `localhost` listener is not advertised because LAN clients cannot reach it.
...
- `Cache:BonjourEnabled`: enables Bonjour/mDNS publication for a non-loopback gRPC listener. Defaults to `true`, but the default `http://localhost:50051` gRPC listener is intentionally not advertised; bind `Cache:GrpcListenUrl` to `0.0.0.0`, `[::]`, or a trusted LAN IP to make the server discoverable. Wildcard listeners publish automatically detected LAN addresses; concrete LAN IP listeners publish only that configured IP.
