...
`Package.swift` also removes upstream test target declarations because this vendored copy intentionally excludes the upstream `Tests` directory. Keeping those test targets makes SwiftPM validate source paths that are not present in the vendor tree.
